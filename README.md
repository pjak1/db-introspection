...
- `db_list_indexes(connection="PROJECT_A/DEV/schema_a", schema="some_schema", table="some_table")` lists indexes (uniqueness, primary-key flag, type, indexed columns); `table` is optional. Indexed columns come back split: `columns` holds the key columns in key order and `included_columns` the INCLUDE/covering columns (null on Oracle, which has no such clause) — an index covers a query only when the filtered/joined columns are in `columns`. Dialect extras: `clustering_factor`, `distinct_keys`, `status`, `is_functional` on Oracle; `filter_definition`, `is_disabled` on SQL Server; `is_partial`, `is_valid`, `definition` on PostgreSQL.
- `db_index_usage(connection="PROJECT_A/DEV/schema_a", schema="some_schema", table="some_table", include_fragmentation=False)` reports how much each index is read versus the writes it costs (`reads`, `writes_overhead`, `last_used`, `size_bytes`, `never_used`, `stats_since`); `table` and `include_fragmentation` are optional. **Always read `stats_since` before acting on `never_used`** — the counters restart from zero on restart/failover and on index drop-recreate, so an index unused since yesterday may be critical to a monthly job; and never drop a unique index on this evidence, it enforces a constraint regardless of reads. SQL Server adds the seek/scan/lookup split and (with `include_fragmentation`, markedly slower) `fragmentation_pct`; PostgreSQL adds tuple counters and warns when `track_counts` is off. Not available on Oracle (`DBA_INDEX_USAGE` needs `SELECT_CATALOG_ROLE`, `V$OBJECT_USAGE` needs write-enabled index monitoring), where it returns empty data with a warning.
- `db_column_stats(connection="PROJECT_A/DEV/schema_a", schema="some_schema", table="some_table", column="some_column", include_histogram=False)` returns the optimizer statistics behind the row estimates (`kind`, `distinct_estimate`, `null_fraction`, `avg_width`, `last_analyzed`, `source`); `column` and `include_histogram` are optional. `kind="extended"` marks a multi-column statistic — without one the optimizer multiplies the selectivities of correlated predicates and can estimate a handful of rows where there are thousands. Oracle returns exact gathered counts plus column groups (`ALL_STAT_EXTENSIONS`); PostgreSQL adds `correlation` and `CREATE STATISTICS` objects, and a **negative `distinct_estimate` there is a fraction of the row count, not a count**; SQL Server has no distinct/null count in its catalog, so those are null unless `include_histogram=True` derives them from `dm_db_stats_histogram`, but it always returns `modification_counter` (rows changed since the statistic was updated), the quickest way to spot stale statistics.
...
The `db_top_queries`, `db_health_check`, `db_index_usage` and `db_column_stats` tools read instance-level catalogs/DMVs and therefore depend on the privileges granted to the connection's DB user; they are designed to degrade gracefully rather than fail. A tool that has no data source on a given dialect returns `ok:true` with empty data, `status="not_available"` and a warning naming the missing grant — never an error.
...
9a. When tuning a specific query: `db_column_stats` for selectivity and statistics freshness, `db_index_usage` for whether an index earns its keep, `db_list_indexes` for what an index actually covers. All three are privilege-dependent and degrade gracefully.
